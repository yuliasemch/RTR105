...
y_second_derivative = (f(cos(x+delta_x) - cos(x)) / delta_x
                       plt.plot(x, y_second_derivative, color = "#800080")
...
y_second_derivative_build_from_array = []
for i in range(N-1):
    temp = ( y[i+1] - y[i] ) / (delta_x)
    #temp = ( y[i+1] - y[i] ) / (x[i+1] - x[i] )
    y_second_derivative_build_from_array.append(temp)
    #print(i,x[i],x[i+1],y[i],y[i+1],temp,y_second_derivative_build_from_array)

plt.plot(x[0:N-1], y_second_derivative_build_from_array, color = "#BC8F8F")
plt.legend(['$sin(x)$','$sin\'(x)$','$sin\'(x)$ - build from array'])
plt.show()
